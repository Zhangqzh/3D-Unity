...
public class UserGUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(Screen.width / 2 - 30, Screen.width / 2 - 350, 100, 100), "Hit UFO", title);
        GUI.Label(new Rect(Screen.width / 2 - 100, Screen.width / 2 - 280, 100, 100),
            "Round " + Director.getInstance().currentSceneController.GetRound().ToString(), scoreStyle);
        GUI.Label(new Rect(Screen.width / 2 + 60, Screen.width / 2 - 280, 50, 50),
            "Score: " + Director.getInstance().currentSceneController.GetScore().ToString(), scoreStyle);
        // 按下按钮的操作
        if (GUI.Button(new Rect(Screen.width / 2 - 30, Screen.width / 2 - 210, 100, 50), buttonText, buttonStyle))
        {
            if (buttonText == "Pause")
            {
